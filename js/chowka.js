...
import {
  ...
  getChowkaDateBounds,
  isValidChowkaDate,
} from "./chowka-logic.js";

document.addEventListener("DOMContentLoaded", () => {
  const API_URL =
    "https://script.google.com/macros/s/AKfycbxprS-2iO3gQTSpFci2-DKhgOEZcbFS-Cc_WFpNw5nULHj6P4GLM1OiJn0EfN7-iSSu4Q/exec";

  const dateInput = document.getElementById("chowka-date");

  const title = document.getElementById("selected-date-title");

  const subtitle = document.getElementById("selected-date-subtitle");

  const tableSection = document.getElementById("chowka-table-section");

  const tbody = document.getElementById("chowka-table-body");

  const tableDate = document.getElementById("table-date-label");

  const count = document.getElementById("chowka-count");

  const empty = document.getElementById("chowka-empty");

  const emptyMsg = document.getElementById("empty-message");

  const status = document.getElementById("chowka-status");

  const buttons = document.querySelectorAll(".maharaj-card");

  const closeDetailsButton = document.getElementById(
    "close-maharaj-details",
  );
  ...
  const dateBounds = getChowkaDateBounds(today);

  dateInput.max = dateBounds.max;

  dateInput.min = dateBounds.min;

  dateInput.value = toInputDate(today);

  renderDate(dateInput.value);
  ...
  dateInput.addEventListener("change", () => {
    ...
    if (!isValidChowkaDate(dateInput.value, today)) {
      showStatus(
  ...
  buttons.forEach((button) => {
    button.addEventListener("click", () => {
      selectMaharaj(button.dataset.maharaj);
    });
  });
  ...
  async function renderDate(key) {
    ...
    try {
      ...
      /*
       * IMPORTANT:
       * Do not use the same variable name as the imported
       * bothMaharajUpdated() function.
       *
       * We need two DISTINCT Maharaj names before the
       * remaining unconfirmed rows become blank.
       */
      const hasBothMaharajUpdated =
        bothMaharajUpdated(result.rows);
  ...
  async function selectMaharaj(key) {
    ...
    try {
      ...
      if (
        ...
        padgahan.innerHTML = placeholder();

        tyaag.innerHTML = placeholder();

        kram.innerHTML = placeholder();
      } else {
        detailName.textContent =
          result.data.name;
